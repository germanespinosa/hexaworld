...
const Cell & Habit_training_prey::start(unsigned int t) {
    episodes++;
    //pick the next habit
    ...
    vector<unsigned int> visits(_sizes[_current_habit]);
    ...
    unsigned int action_count = habit.values[_start_cell_index].actions.size();
    vector<unsigned int > action_visits(action_count);
    for (unsigned int i = 0 ; i < action_count ; i++) action_visits[i] = habit.values[_start_cell_index].actions[i].visits;
...
void Habit_training_prey::update(const State &state) {
    ...
    auto &habit = _habits[_current_habit];
    int cell_index = habit.nodes.find(cell());
...
void Habit_training_prey::end(Episode_result r, unsigned int l) {

...
std::vector<Coordinates> const posible_actions = {{0,0},{1,1},{2,0},{1,-1},{-1,-1},{-2,0},{-1,1}};
...
int main (int argc, char *args[]) {
    ...
    vector<Coordinates> conns {{0,0},{2,0},{1,1},{1,-1},{0,-2},{-1,-1},{-1,1}};
    for (int i = 0; i < world.size(); i++){
        for (int c =0; c < conns.size();c++){
            int cc = world.find(world[i].coordinates+conns[c]);
            if (cc>=0){
                if (!world[cc].occluded)
                    world[i].connections.push_back(cc);
            }
        }
    }
    world.save(filepath);
